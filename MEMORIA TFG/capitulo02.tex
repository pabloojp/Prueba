...
\subsubsection*{Función Sigmoide}
    ...
    \end{equation}
    Proyecta salidas de números reales de entrada al intervalo \([0, 1]\). Sin embargo, tiene problemas como la saturación del gradiente (cuando los valores se aproximan a 0 o 1, el gradiente tiende a 0, lo que afecta en la actualización de los pesos) o los pesos positivos de forma continua (la esperanza de la función de salida no es 0), lo que puede llevar a una convergencia lenta.
...
\subsubsection*{Función Sigmoide Dura}

    La función sigmoide dura (\textit{hard-sigmoid}) es una aproximación simplificada de la sigmoide que reduce los problemas de saturación del gradiente:
...
\subsubsection*{Función Tangente Hiperbólica (tanh)}
    ...
    La función tangente hiperbólica (\textit{tanh}) es similar a la sigmoide pero escala las salidas al rango \([-1, 1]\):

\begin{equation}
    \tanh(x) = \frac{2}{1 + e^{-2x}} - 1
\end{equation}

Aunque también sufre de saturación del gradiente, \textit{tanh} centra las salidas alrededor de cero, lo que puede acelerar el entrenamiento. 




\subsubsection*{Función Unidad Lineal Rectificada (ReLU)}
...
\subsubsection*{Función Unidad Lineal Exponencial (ELU)}
...
\subsubsection*{Función Unidad Lineal Exponencial Escalada (SELU)}

La función SELU es una versión modificada de ELU, que introduce un parámetro adicional \(\lambda\) para escalar la salida:
...
donde \(\sigma(x)\) es la función sigmoide. La Figura \ref{img: activf} muestra Swish con \(\beta = 3\) y SiLU con \(\beta = 1\).


\subsubsection*{Función Mish}
...
Las funciones de activación juegan un papel esencial en la formación de redes neuronales al permitir que las redes aprendan y modelen relaciones complejas. Desde funciones clásicas como la sigmoide y \textit{tanh}, hasta variantes más recientes como ReLU, LReLU, ELU, y SELU, cada función tiene sus propios beneficios y limitaciones. La elección de la función de activación adecuada puede influir significativamente en el rendimiento del modelo. Además, permiten resolver el desvanecimiento del gradiente durante el proceso de aprendizaje, evitando que el gradiente se acerque a cero, ya que es necesario que sean derivables \citep{pajares2021aprendizaje}.
...
Aquí, \(p(x)\) es la distribución verdadera (generalmente representada como \textit{one-hot} para una categoría) y \(q(x)\) es la distribución predicha. Esta función penaliza las predicciones que se desvían de las probabilidades verdaderas, con valores más altos indicando mayores discrepancias. 
...
Para implementarlo en la práctica, frameworks como \texttt{Keras} ofrecen callbacks que facilitan este proceso. A continuación se presenta un ejemplo de código que utiliza \texttt{EarlyStopping} para detener el entrenamiento cuando no se observa ninguna mejora en el error de validación:
...
En este ejemplo, EarlyStopping detiene el entrenamiento después de un número definido de épocas sin mejora ($patience = 10$), restaurando los pesos que lograron el mejor desempeño \citep{geron2022hands}.
...
Para calcular este umbral, se utilizan las puntuaciones de decisión obtenidas del clasificador. Primero, se obtienen estas puntuaciones usando la función \texttt{decision\_function()} del clasificador. Luego, se emplea la función \texttt{precision\_recall\_curve()} de Scikit-Learn para calcular la precisión y la sensibilidad para todos los umbrales posibles. Finalmente, se selecciona el umbral que proporciona la precisión deseada. La Figura \ref{img: pr_vs_threshold} ilustra cómo se comportan la precisión y la sensibilidad en función del umbral \citep{geron2022hands}.
